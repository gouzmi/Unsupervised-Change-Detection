...
def img_to_patches_to_tensor(index, list_id, image, patch_size):
    id = list_id[index]
    i = int(id/(len(image[0])-(patch_size-1)))
    j = id % (len(image[0])-(patch_size-1))
    patch = np.reshape(image[i:(i+patch_size), j:(j+patch_size)], (-1,patch_size, patch_size))
...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return img_tensor1, img_tensor2, self.id[index]
